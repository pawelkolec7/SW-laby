...
def read_sensor_data(sensor, pin, num_readings):
    ...
    for _ in range(num_readings):
        ...
        if temperature is not None:
            ...
            print(f"Odczyt temperatury: {temperature:.1f}°C")
        else:
...
def display_database_contents(cursor):
    ...
    print("\nOstatnie 5 pomiarów:")
    for row in rows:
        print(f"Id: {row[0]}, Data i godzina: {row[1]}, Średni pomiar: {row[2]:.2f}°C")
...
def main():
    ...
    for _ in range(MEASUREMENTS_COUNT):
        ...
        if average is not None:
            ...
            conn.commit()
            print(f"Średnia temperatura: {average:.2f}°C zapisana do bazy.")
        else:
